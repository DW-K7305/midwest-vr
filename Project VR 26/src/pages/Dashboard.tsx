...
import { HeadsetPanel } from "@/components/HeadsetPanel";
...
export function Dashboard() {
  ...
  // Phase 44: which headset's command panel is open. Click any DeviceCard
  // to open it; close to return to the grid.
  ...
  return (
    <div className="flex flex-col h-full">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
              {all.map((d) => (
                <DeviceCard
                  key={d.serial}
                  device={d}
                  onClick={() => setOpenSerial(d.serial)}
                />
              ))}
            </div>
            <WirelessPanel />
      ...
      {/* Per-headset command panel — slide-in dialog */}
      <HeadsetPanel
        serial={openSerial}
        onOpenChange={(open) => !open && setOpenSerial(null)}
      />
    </div>
  );
}
