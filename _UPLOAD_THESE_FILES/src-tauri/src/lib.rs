...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            // Self-healing wireless. Two cooperating mechanisms:
            //
            ...
            //      when ANY paired headset is currently offline (so wake-
            //      from-sleep is detected quickly), 30s when everything is
            //      already connected (so we don't burn cycles for no reason).
            //
            //   2. ON-DEMAND TRIGGER. The frontend listens for window focus
            ...
            //
            // Net result: pair once over USB, and the headset stays usable
            // through sleep, wake, network reconnects — without user action.
            ...
            tauri::async_runtime::spawn(async move {
                ...
                loop {
                    ...
                    if !paired_ips.is_empty() {
                        let connected = wireless::reconnect_all(&adb_path, &paired_ips).await;
                        // If anything is missing, poll again soon so wake-from-sleep
                        // shows up fast.
                        if connected.len() < paired_ips.len() {
                            sleep_secs = 8;
                        }
                    }
